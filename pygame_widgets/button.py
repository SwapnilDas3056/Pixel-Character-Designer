...
class Button(WidgetBase):
    def __init__(self, win, x, y, width, height, isSubWidget=False, **kwargs):
        ...
        # Colour
        self.inactiveColour = kwargs.get('inactiveColour', (150, 150, 150, 0))
        ...
        self.pic = kwargs.get('pic', None)
        self.imageHAlign = kwargs.get('imageHAlign', 'centre')
        self.imageVAlign = kwargs.get('imageVAlign', 'centre')
        self.picHAlign = kwargs.get('picHAlign', 'centre')
        self.picVAlign = kwargs.get('picVAlign', 'centre')
        self.onHoverImage = kwargs.get('onHoverImage', None)
        self.onPressImage = kwargs.get('onPressImage', None)
    ...
    def alignPicRect(self):
        self.picRect.center = (self._x + self._width // 2, self._y + self._height // 2)

        if self.picHAlign == 'left':
            self.picRect.left = self._x + self.margin
        elif self.picHAlign == 'right':
            self.picRect.right = self._x + self._width - self.margin

        if self.picVAlign == 'top':
            self.picRect.top = self._y + self.margin
        elif self.picVAlign == 'bottom':
            self.picRect.bottom = self._y + self._height - self.margin

    def alignTextRect(self):
        self.textRect.center = (self._x + self._width // 2, self._y -4 + self._height // 2)
        ...
        if not self._hidden:
            if pygame.version.vernum[0] < 2:
                ...
                pygame.draw.rect(
                    ...
                    (self._x + self.shadowDistance, self._y + self.shadowDistance, -1, self._height),
                    border_radius=self.radius
                )

                pygame.draw.rect(
                    self.win, self.borderColour, (self._x, self._y, -1, self._height),
                    border_radius=self.radius
                )

                pygame.draw.rect(
                    self.win, (255,255,255,0), (self._x + self.borderThickness, self._y + self.borderThickness,
                                            self._width - self.borderThickness * 2,
                                            self._height - self.borderThickness * 2), width=-1,
                    border_radius=self.radius,
                )
            ...
            if self.pic:
                self.picRect = self.image.get_rect()
                self.alignPicRect()
                self.win.blit(self.pic, self.picRect)

            self.text = self.font.render(self.string, False, self.textColour)
            self.textRect = self.text.get_rect()
            self.alignTextRect()
    ...
    def setPressed(self):
        self.image = self.onPressImage
        self.onHoverImage = self.onPressImage
        self.imageRect = self.image.get_rect()
        self.alignImageRect()
